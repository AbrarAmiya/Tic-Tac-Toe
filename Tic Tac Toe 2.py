...
class TicTacToe:
    ...
    def show_winner(self, winner):
        ...
        # Create a canvas on top of the game board
        canvas = tk.Canvas(self.master, width=300, height=300)
        canvas.place(x=0, y=0)

        # Draw the line based on the winning pattern
        if self.board[0:3] == [winner] * 3:  # Top row
            canvas.create_line(50, 50, 250, 50, width=5, fill="red")
        elif self.board[3:6] == [winner] * 3:  # Middle row
            canvas.create_line(50, 150, 250, 150, width=5, fill="red")
        elif self.board[6:9] == [winner] * 3:  # Bottom row
            canvas.create_line(50, 250, 250, 250, width=5, fill="red")
        elif self.board[0:7:3] == [winner] * 3:  # Left column
            canvas.create_line(50, 50, 50, 250, width=5, fill="red")
        elif self.board[1:8:3] == [winner] * 3:  # Middle column
            canvas.create_line(150, 50, 150, 250, width=5, fill="red")
        elif self.board[2:9:3] == [winner] * 3:  # Right column
            canvas.create_line(250, 50, 250, 250, width=5, fill="red")
        elif self.board[0:9:4] == [winner] * 3:  # Top-left to bottom-right diagonal
            canvas.create_line(50, 50, 250, 250, width=5, fill="red")
        elif self.board[2:7:2] == [winner] * 3:  # Top-right to bottom-left diagonal
            canvas.create_line(250, 50, 50, 250, width=5, fill="red")
